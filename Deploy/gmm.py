...
class GMMClusteringApp:
    ...
    def run_clustering(self):
        st.title('Clustering Scores with GMM')
        if self.kmax is None:
            st.write("Please provide a value for kmax.")
            return

        st.write(f"Running GMM clustering with kmax={self.kmax}...")

        fig, axes = plt.subplots(nrows=1, ncols=self.kmax-1, figsize=(25, 5))
        ks = range(2, self.kmax + 1)

        for k in ks:
            gmm = GaussianMixture(n_components=k, random_state=42).fit(self.x)
...
# Usage
# app = GMMClusteringApp(df, kmax=10)
# app.run_clustering()
